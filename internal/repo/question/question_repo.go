...
func (qr *questionRepo) SitemapQuestions(ctx context.Context, page, pageSize int) (
	...
	cacheKey := fmt.Sprintf(constant2.SiteMapQuestionCacheKeyPrefix, page)
	...
	if err := qr.Cache.Set(ctx, cacheKey, string(cacheDataByte), constant2.SiteMapQuestionCacheTime).Err(); err != nil {
		log.Error(err)
	}
	return questionIDList, nil
}
...
func (qr *questionRepo) updateSearch(ctx context.Context, questionID string) (err error) {
	...
	content := &plugin.SearchContent{
		...
		Type:        constant2.QuestionObjectType,
