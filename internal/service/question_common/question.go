...
import (
	...
	"github.com/apache/incubator-answer/commons/utils/checker"
...
func (qs *QuestionCommon) Info(ctx context.Context, questionID string, loginUserID string) (showinfo *schema.QuestionInfo, err error) {
	...
	if showinfo.Status == 2 {
		...
		metainfo, err = qs.metaService.GetMetaByObjectIdAndKey(ctx, dbinfo.ID, entity2.QuestionCloseReasonKey)
		if err != nil {
			log.Error(err)
		} else {
			// metainfo.Value
...
func (qs *QuestionCommon) CloseQuestion(ctx context.Context, req *schema.CloseQuestionReq) error {
	...
	err = qs.metaService.AddMeta(ctx, req.ID, entity2.QuestionCloseReasonKey, string(closeMeta))
	if err != nil {
		return err
	}
...
func (qs *QuestionCommon) SitemapCron(ctx context.Context) {
	...
	for i := 1; i <= totalPages; i++ {
		_, err = qs.questionRepo.SitemapQuestions(ctx, i, constant2.SitemapMaxSize)
		if err != nil {
			log.Errorf("get site map question error: %v", err)
			return
		}
...
func (qs *QuestionCommon) SetCache(ctx context.Context, cachekey string, info interface{}) error {
	...
	err = qs.Cache.Set(ctx, cachekey, string(infoStr), schema.DashboardCacheTime).Err()
	if err != nil {
		return errors.InternalServer(reason.UnknownError).WithError(err).WithStack()
	}
	return nil
