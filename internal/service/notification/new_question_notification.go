...
func (ns *ExternalNotificationService) getNewQuestionSubscribers(ctx context.Context, msg *schema.ExternalNotificationMsg) (
	...
	notificationConfigs, err := ns.userNotificationConfigRepo.GetBySource(ctx, constant2.AllNewQuestionSource)
	if err != nil {
		return nil, err
	}
...
func (ns *ExternalNotificationService) checkSendNewQuestionNotificationEmailLimit(ctx context.Context, userID string) bool {
	...
	old, err := data.RedisClient.Get(ctx, key).Int64()
	if err != nil {
		log.Error(err)
		return false
	}
	...
	err = data.RedisClient.Incr(ctx, key).Err()
	if err != nil {
		log.Error(err)
	}
	return false
