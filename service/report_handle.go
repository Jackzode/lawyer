...
func (rh *ReportHandle) HandleObject(ctx context.Context, reported *entity.Report, req schema.ReportHandleReq) (err error) {
	...
	case "question":
		...
		case reasonDeleteCfg.ID:
			err = QuestionCommonServicer.RemoveQuestion(ctx, &schema.RemoveQuestionReq{ID: objectID})
		case reasonCloseCfg.ID:
			err = QuestionCommonServicer.CloseQuestion(ctx, &schema.CloseQuestionReq{
				ID:        objectID,
	...
	case "answer":
		switch req.FlaggedType {
		case reasonDeleteCfg.ID:
			err = QuestionCommonServicer.RemoveAnswer(ctx, objectID)
		}
	case "comment":
		switch req.FlaggedType {
		case reasonCloseCfg.ID:
