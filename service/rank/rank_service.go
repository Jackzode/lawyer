...
func (rs *RankService) CheckOperationPermission(ctx context.Context, userID string, action string, objectID string) (
	...
	userInfo, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, userID)
	if err != nil {
		return false, err
	}
	if !exist {
	...
	if len(objectID) > 0 {
		objectInfo, err := services.ObjService.GetInfo(ctx, objectID)
		if err != nil {
			return can, err
		}
...
func (rs *RankService) getUserPowerMapping(ctx context.Context, userID string) (powerMapping map[string]bool) {
	...
	userRole, err := services.UserRoleRelService.GetUserRole(ctx, userID)
	if err != nil {
		log.Error(err)
		return powerMapping
	}
	powers, err := services.RolePowerRelService.GetRolePowerList(ctx, userRole)
	if err != nil {
		log.Error(err)
		return powerMapping
	}
...
func (rs *RankService) decorateRankPersonalPageResp(
	...
	for _, userRankInfo := range userRankPage {
		...
		objInfo, err := services.ObjService.GetInfo(ctx, userRankInfo.ObjectID)
		if err != nil {
			log.Error(err)
			continue
		}
