...
func (us *UserAdminService) UpdateUserRole(ctx context.Context, req *schema.UpdateUserRoleReq) (err error) {
	...
	err = services.UserRoleRelService.SaveUserRole(ctx, req.UserID, req.RoleID)
	if err != nil {
		return err
	}

	services.AuthService.RemoveUserAllTokens(ctx, req.UserID)
	return
}
...
func (us *UserAdminService) AddUser(ctx context.Context, req *schema.AddUserReq) (err error) {
	...
	userInfo.Username, err = services.UserCommon.MakeUsername(ctx, userInfo.DisplayName)
	if err != nil {
		return err
	}
	userInfo.MailStatus = entity.EmailStatusAvailable
...
func (us *UserAdminService) GetUserPage(ctx context.Context, req *schema.GetUserPageReq) (pageModel *pager.PageModel, err error) {
	...
	avatarMapping := services.SiteInfoCommonService.FormatListAvatar(ctx, users)
...
func (us *UserAdminService) setUserRoleInfo(ctx context.Context, resp []*schema.GetUserPageResp) {
	...
	userRoleMapping, err := services.UserRoleRelService.GetUserRoleMapping(ctx, userIDs)
	if err != nil {
		log.Error(err)
		return
	}
...
func (us *UserAdminService) GetUserActivation(ctx context.Context, req *schema.GetUserActivationReq) (
	...
	code := uuid.NewString()
	services.EmailService.SaveCode(ctx, code, data.ToJSONString())
...
func (us *UserAdminService) SendUserActivation(ctx context.Context, req *schema.SendUserActivationReq) (err error) {
	...
	go services.EmailService.SendAndSaveCode(ctx, user.EMail, title, body, code, data.ToJSONString())
	return nil
}
