...
func (rs *ReportAdminService) ListReportPage(ctx context.Context, dto schema.GetReportListPageDTO) (pageModel *pager.PageModel, err error) {
	...
	// flagged users
	flaggedUsers, err = services.UserCommon.BatchUserBasicInfoByID(ctx, flaggedUserIds)
	if err != nil {
		return nil, err
	}

	// flag users
	users, err = services.UserCommon.BatchUserBasicInfoByID(ctx, userIds)
	if err != nil {
		return nil, err
	}
	for _, r := range resp {
...
func (rs *ReportAdminService) HandleReported(ctx context.Context, req schema.ReportHandleReq) (err error) {
	...
	if err = services.ReportHandle.HandleObject(ctx, reported, req); err != nil {
		return
	}
...
func (rs *ReportAdminService) decorateReportResp(ctx context.Context, resp *schema.GetReportListPageResp) {
	...
	objectInfo, err := services.ObjService.GetInfo(ctx, resp.ObjectID)
	if err != nil {
		log.Error(err)
		return
	}
