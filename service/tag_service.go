...
func (ts *TagService) RemoveTag(ctx context.Context, req *schema.RemoveTagReq) (err error) {
	...
	tagCount, err := TagCommonServicer.CountTagRelByTagID(ctx, req.TagID)
	if err != nil {
		return err
	}
	if tagCount > 0 {
...
func (ts *TagService) GetTagInfo(ctx context.Context, req *schema.GetTagInfoReq) (resp *schema.GetTagResp, err error) {
	...
	if len(req.ID) > 0 {
		tagInfo, exist, err = TagCommonServicer.GetTagByID(ctx, req.ID)
	} else {
		tagInfo, exist, err = TagCommonServicer.GetTagBySlugName(ctx, req.Name)
	}
	...
	if tagInfo.MainTagID > 0 {
		tagInfo, exist, err = TagCommonServicer.GetTagByID(ctx, converter.IntToString(tagInfo.MainTagID))
		if err != nil {
			return nil, err
		}
		if !exist {
...
func (ts *TagService) GetTagsBySlugName(ctx context.Context, tagNames []string) ([]*schema.TagItem, error) {
	...
	tagListInDB, err := TagCommonServicer.GetTagListByNames(ctx, tagNames)
	if err != nil {
		return tagList, err
	}
...
func (ts *TagService) GetFollowingTags(ctx context.Context, userID string) (
	...
	tagList, err := TagCommonServicer.GetTagListByIDs(ctx, objIDs)
	if err != nil {
		return nil, err
	}
	for _, t := range tagList {
		...
		if t.MainTagID > 0 {
			mainTag, exist, err := TagCommonServicer.GetTagByID(ctx, converter.IntToString(t.MainTagID))
			if err != nil {
				return nil, err
			}
			if exist {
...
func (ts *TagService) GetTagSynonyms(ctx context.Context, req *schema.GetTagSynonymsReq) (
	...
	tag, exist, err := TagCommonServicer.GetTagByID(ctx, req.TagID)
	if err != nil {
		return
	}
	if !exist {
...
func (ts *TagService) UpdateTagSynonym(ctx context.Context, req *schema.UpdateTagSynonymReq) (err error) {
	...
	mainTagInfo, exist, err := TagCommonServicer.GetTagByID(ctx, req.TagID)
	if err != nil {
		return err
	}
	if !exist {
	...
	tagListInDB, err := TagCommonServicer.GetTagListByNames(ctx, addSynonymTagList)
	if err != nil {
		return err
	}
	...
	if len(needAddTagList) > 0 {
		err = TagCommonServicer.AddTagList(ctx, needAddTagList)
		if err != nil {
			return err
		}
		// update tag revision
...
func (ts *TagService) GetTagWithPage(ctx context.Context, req *schema.GetTagWithPageReq) (pageModel *pager.PageModel, err error) {
	...
	tags, total, err := TagCommonServicer.GetTagPage(ctx, page, pageSize, tag, req.QueryCond)
	if err != nil {
		return
	}
