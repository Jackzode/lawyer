...
func (as *ActivityService) getTimelineMainObjInfo(ctx context.Context, objectID string) (
	...
	objInfo, err := services.ObjService.GetInfo(ctx, objectID)
	if err != nil {
		return nil, err
	}
	...
	if len(objInfo.ObjectCreatorUserID) > 0 {
		...
		userBasicInfo, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, objInfo.ObjectCreatorUserID)
		if err != nil {
			return nil, err
		}
		if exist {
...
func (as *ActivityService) formatTimelineUserInfo(ctx context.Context, timeline []*schema.ActObjectTimeline) {
	...
	userInfoMapping, err := services.UserCommon.BatchUserBasicInfoByID(ctx, userIDs)
	if err != nil {
		log.Error(err)
		return
	}
...
func (as *ActivityService) getOneObjectDetail(ctx context.Context, revisionID string) (
	...
	objInfo, err := services.ObjService.GetInfo(ctx, revision.ObjectID)
	if err != nil {
		return nil, err
	}
