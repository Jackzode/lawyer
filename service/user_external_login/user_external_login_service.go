...
func (us *UserExternalLoginService) ExternalLogin(
	...
	if exist {
		...
		if exist && oldUserInfo.Status != entity.UserStatusDeleted {
			...
			accessToken, _, err := services.UserCommon.CacheLoginUserInfo(
	...
	if err := services.UserNotificationConfigService.SetDefaultUserNotificationConfig(ctx, []string{oldUserInfo.ID}); err != nil {
...
func (us *UserExternalLoginService) registerNewUser(ctx context.Context,
	...
	userInfo.Username, err = services.UserCommon.MakeUsername(ctx, externalUserInfo.Username)
	if err != nil {
		log.Error(err)
...
func (us *UserExternalLoginService) ExternalLoginBindingUserSendEmail(
	...
	if !exist {
		...
		resp.AccessToken, _, err = services.UserCommon.CacheLoginUserInfo(
