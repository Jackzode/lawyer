...
func (qs *QuestionCommon) InviteUserInfo(ctx context.Context, questionID string) (inviteList []*schema.UserBasicInfo, err error) {
	...
	if dbinfo.InviteUserID != "" {
		...
		if err == nil {
			inviteUserInfoMap, err := services.UserCommon.BatchUserBasicInfoByID(ctx, InviteUserIDs)
			if err == nil {
...
func (qs *QuestionCommon) Info(ctx context.Context, questionID string, loginUserID string) (showinfo *schema.QuestionInfo, err error) {
	...
	tagmap, err := services.TagCommonService.GetObjectTag(ctx, questionID)
	if err != nil {
		return showinfo, err
	}
	...
	ids, err := services.AnswerCommon.SearchAnswerIDs(ctx, loginUserID, dbinfo.ID)
	if err != nil {
	...
	collectedMap, err := services.CollectionCommon.SearchObjectCollected(ctx, loginUserID, []string{dbinfo.ID})
	if err != nil {
		return nil, err
	}
...
func (qs *QuestionCommon) FormatQuestionsPage(
	...
	tagsMap, err := services.TagCommonService.BatchGetObjectTag(ctx, questionIDs)
	if err != nil {
...
func (qs *QuestionCommon) FormatQuestions(ctx context.Context, questionList []*entity.Question, loginUserID string) ([]*schema.QuestionInfo, error) {
	...
	tagsMap, err := services.TagCommonService.BatchGetObjectTag(ctx, objectIds)
	if err != nil {
		return list, err
	}
	...
	collectedMap, err := services.CollectionCommon.SearchObjectCollected(ctx, loginUserID, objectIds)
	if err != nil {
		return nil, err
	}
	for _, item := range list {
