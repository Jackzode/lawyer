...
func (cs *CommentService) AddComment(ctx context.Context, req *schema.AddCommentReq) (
	...
	// get user info
	userInfo, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, resp.UserID)
	if err != nil {
		return nil, err
	}
	if exist {
...
func (cs *CommentService) addCommentNotification(
	...
	if len(resp.ReplyUserID) > 0 && resp.ReplyUserID != req.UserID {
		replyUser, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, resp.ReplyUserID)
		if err != nil {
			return nil, err
		}
		if exist {
...
func (cs *CommentService) GetComment(ctx context.Context, req *schema.GetCommentReq) (resp *schema.GetCommentResp, err error) {
	...
	if len(resp.UserID) > 0 {
		commentUser, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, resp.UserID)
		if err != nil {
			return nil, err
		}
		if exist {
...
func (cs *CommentService) convertCommentEntity2Resp(ctx context.Context, req *schema.GetCommentWithPageReq,
	...
	if len(commentResp.UserID) > 0 {
		commentUser, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, commentResp.UserID)
		if err != nil {
			return nil, err
		}
		if exist {
	...
	if len(commentResp.ReplyUserID) > 0 {
		replyUser, exist, err := services.UserCommon.GetUserBasicInfoByID(ctx, commentResp.ReplyUserID)
		if err != nil {
			return nil, err
		}
		if exist {
...
func (cs *CommentService) GetCommentPersonalWithPage(ctx context.Context, req *schema.GetCommentPersonalWithPageReq) (
	...
	for _, comment := range commentList {
		...
		if len(comment.ObjectID) > 0 {
			objInfo, err := services.ObjService.GetInfo(ctx, comment.ObjectID)
			if err != nil {
				log.Error(err)
			} else {
...
func (cs *CommentService) notificationQuestionComment(ctx context.Context, questionUserID,
	...
	commentUser, _, _ := services.UserCommon.GetUserBasicInfoByID(ctx, commentUserID)
	if commentUser != nil {
