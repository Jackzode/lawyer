...
func initTestDataSource(dbSetting TestDBSetting) error {
	...
	newData, dbCleanUp, err := data2.NewData(dbEngine, newCache)
	if err != nil {
		return err
	}
...
func initDatabase(dbSetting TestDBSetting) (dbEngine *xorm.Engine, err error) {
	dataConf := &data2.Database{Driver: dbSetting.Driver, Connection: dbSetting.Connection}
	dbEngine, err = data2.NewDB(true, dataConf)
	if err != nil {
...
func initCache() (newCache cache.Cache, err error) {
	newCache, _, err = data2.NewCache(&data2.RedisConf{})
	return newCache, err
}
